...
Install dependencies and start the backend. If you use provider credentials (AWS), start the server from the shell where those env vars or profiles are configured so Terraform can access them:
...
- Start the backend from a shell with required cloud provider credentials visible (env vars, profiles).
- The backend only runs read-only Terraform commands (`init`, `plan`, `show -json`, `graph`).

---

## Frontend (run UI)

In a separate terminal, install and run the frontend dev server:
...
Open the Vite URL shown by the command (typically http://localhost:5173).
...
- Do NOT commit credentials or state files. See the `.gitignore` for recommended ignores.
...
- Design tokens and global styles are defined in `frontend/src/styles/tokens.css` and `frontend/src/styles/global.css`. See `frontend/README_DESIGN.md` for design system details.
...
- Tighten the repository `.gitignore` (I can update it now),
- Add convenience npm scripts to start backend+frontend together, or
- Create a quick developer checklist / CONTRIBUTING.md.

---

License / Next Steps

This project is a local, non-destructive tool. Consider adding module grouping or visual improvements to the graph; I can add node grouping by module path next if you'd like.
# Terraform UI MVP (Local, Read-Only)
